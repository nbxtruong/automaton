...
public class Edge extends JoinPointBase {
	...
	public void draw(Graphics2D ga) {
		...
		if (getSource().equals(getDest())
				|| getSource().verifyShapeExists(point)) {
			if (isDrawing()) {
				distance = Math.sqrt(Math.pow(
						(point.getX() - sourcePoint.getX()), 2)
						+ Math.pow((point.getY() - sourcePoint.getY()), 2));
			...
			ga.draw(new Line2D.Float(new Point2D.Double(x, y),
					new Point2D.Double(quar_x, quar_y)));
			ga.draw(new Line2D.Float(new Point2D.Double(x2, y2),
					new Point2D.Double(quar_x2, quar_y2)));
			line = new Line2D.Float(new Point2D.Double(quar_x, quar_y),
					new Point2D.Double(quar_x2, quar_y2));
			drawArrow(ga, line);
