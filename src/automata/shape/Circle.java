...
public class Circle extends BaseShape {
	...
	public void draw(Graphics2D ga) {
		...
		ga.fillOval((int) point.getX() - radius, (int) point.getY() - radius,
				radius * 2, radius * 2);
		ga.setColor(getContrastColor(currentColor));
		int width = ga.getFontMetrics().stringWidth(label);
		ga.drawString(label, (int) (point.getX() - width / 2),
				(int) (point.getY() + 5));
		ga.setColor(current);
	}
	...
	public boolean verifyShapeExists(Point2D point) {
		if (point == null)
			return false;
		if (this.point.getX() + radius > point.getX()
				&& this.point.getX() - radius < point.getX()) {
			if (this.point.getY() + radius > point.getY()
					&& this.point.getY() - radius < point.getY()) {
				return true;
			}
		}
		return false;
