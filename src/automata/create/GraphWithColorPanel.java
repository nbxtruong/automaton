		...
		for (int i = 0; i < colors.size(); i++) {
			...
			button.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					toolbox.repaint();
					/**
					 * We lost focus on editor when select this panel So, we
					 * need to request focus in editor For prevent lost key
					 * request like hold ALT key
					 */
					requestFocus();
				}
			});
	...
	private class ButtonWithCheck extends JButton {
		...
		protected void paintComponent(Graphics g) {
			...
			if (currentColor == color) {
				...
				check.setPosition(new Point2D.Double(point.getX()
						+ getPreferredSize().width / 2 - 7, point.getY() + 25));
