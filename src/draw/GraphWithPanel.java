...
public class GraphWithPanel extends GraphDecorator {
	...
	public GraphWithPanel(Graph decorateComponent) {
		...
		toolbox.setPreferredSize(new Dimension(width, toolbox
	...
	private class ButtonWithIcon extends JButton {
		...
		public ButtonWithIcon(String name, int type) {
			...
			point = getLocation();
			shape.setPosition(new Point2D.Double(point.getX() + width / 2 - 7,
					point.getY() + 30));
			shape.setAsIcon();
		}
		...
		protected void paintComponent(Graphics g) {
			...
			if (shapeTypeSelected == type) {
				...
				check.setPosition(new Point2D.Double(point.getX() + width / 2
						- 7, point.getY() + 30));
				check.draw(ga);
			}
		}
