...
public class GraphWithEditor extends JComponent implements MouseInputListener,
	...
	/**
	 * This is the heart of application This contain all Shapes and Paint area
	 * This design how Shapes connect to another And take care how user
	 * interactive with Shapes
	 */
	...
	/**
	 * Determine which shape will be draw This value must be one of ShapeFactory
	 * type number
	 */
	private int shapeType;

	/**
	 * Determine if user hold ALT key or not True: holding ALT key
	 */
	...
	/**
	 * Determine which point is currently mouse in Change when mouse drag. see
	...
	public void mousePressed(MouseEvent e) {
		...
		/**
		 * This prevent bug when user hold ALT and drag in empty space In this
		 * case, a new Edge is created But this Edge has no Destination Shape
		 * Therefore, we need to set which is destination shape for this Edge
		 * first
		 */
		if (!holdAltKey) {
			setAutomataChanges();
	...
	/**
	 * Remove only JointPoint Reconnect 2 edge between this JoinPoint
	 * 
	 * @param shapeToRemove
	 */
	 ...
	 * @return
	 */
	public static Vector<Edge> getEdgeSource(Shape shapeToFind,
			Vector<Shape> shapes) {
	 ...
	 * @return
	 */
	public static Vector<Edge> getEdgeDest(Shape shapeToFind,
			Vector<Shape> shapes) {
		Vector<Edge> sources = new Vector<>();
		for (Shape shape : shapes) {
			if (shape instanceof Edge) {
	...
	/**
	 * From a given list of Edges Remove all Edges, JointPoints until find
	 * another type of Shape Go from Source to Destination
	 * 
	 * @param edges
	 */
	private void removeEdgeJointPointDest(Vector<Edge> edges) {
		for (Edge e : edges) {
			Edge currentEdge = e;
			removeEdge(e);
			while (shapes.contains(currentEdge.getDest())
					&& currentEdge.getDest() instanceof JoinPoint) {
	...
	/**
	 * From a given list of Edges Remove all Edges, JointPoints until find
	 * another type of Shape Go from Destination to Source
	 * 
	 * @param edges
	 */
	private void removeEdgeJointPointSource(Vector<Edge> edges) {
		for (Edge e : edges) {
			Edge currentEdge = e;
			removeEdge(e);
			while (shapes.contains(currentEdge.getSource())
					&& currentEdge.getSource() instanceof JoinPoint) {
	...
	/**
	 * From a given Shape Create Edge which have source is this Shape
	 * 
	 * @param shape
	 */
	...
	public void mouseDragged(MouseEvent e) {
		 ...
		 */
			...
			/**
			 * If current shape is Edge We just move this shape to new ppoint
			 */
	...
	public void keyPressed(KeyEvent e) {
		...
		/**
		 * If key pressed is space and we are drawing edge So, create join point
		 * here
		 */
		if (e.getKeyCode() == 32 && currentShape instanceof Edge) {
			JoinPoint shape = (JoinPoint) shapeFactory
					.getShape(ShapeFactory.JOIN_POINT);
	...
	/**
	 * Create Edge And also create EdgeLabel for this Edge
	 * 
	 * @return
	 */
	private Shape createEdge() {
		Edge edge = (Edge) shapeFactory.getShape(ShapeFactory.EDGE);
		EdgeLabel edgeLabel = (EdgeLabel) shapeFactory
				.getShape(ShapeFactory.EDGE_LABEL);
	...
	/**
	 * Remove Edge And also remove Edge Label belong to this Edge
	 * 
	 * @param edge
	 */
	...
	/**
	 * From a given Edge Find a Shape which Edge come from - Ignore JoinPoint
	 * 
	 * @param edge
	 * @return
	 */
	public static Shape findSourceShape(Edge edge) {
		...
		while (shape == null) {
			if (currentEdge.getSource() instanceof JoinPoint) {
				currentEdge = (Edge) ((JoinPoint) currentEdge.getSource())
						.getSource();
			} else {
	...
	/**
	 * From a given Edge Find a Shape which Edge come to - Ignore JoinPoint
	 * 
	 * @param edge
	 * @return
	 */
	public static Shape findDestShape(Edge edge) {
		...
		while (shape == null) {
			if (currentEdge.getDest() instanceof JoinPoint) {
				currentEdge = (Edge) ((JoinPoint) currentEdge.getDest())
						.getDest();
			} else {
	...
	/**
	 * From Shape start & Shape end Find the right way (Edge path, include
	 * JoinPoint) which have EdgeLabel as same as given Label And then,
	 * highlight it using color green
	 * 
	 * @param start
	 * @param end
	 * @param shapes
	 * @param label
	 */
	public static void showPath(Shape start, Shape end, Vector<Shape> shapes,
			String label) {
	...
	/**
	 * Using recursive to detect if from given edge, can we go to Shape end If
	 * yes, highlight it using Color green
	 * 
	 * @param edge
	 * @param end
	 * @return
	...
	public static boolean recursiveFindPath(Edge edge, Shape end) {
		if (edge.getDest() instanceof JoinPoint) {
			if (recursiveFindPath(
					(Edge) ((JoinPoint) edge.getDest()).getDest(), end)) {
